...
import { dcpCmdOf, keyFormatFromProperties, wrappingKeyToNativeBytes } from "../args/index.js"
import { UnknownZfsError, type ZfsError } from "../errors/classify.js"
import {
  BadAttachTarget,
  BadPath,
  BadRestore,
...
import { ZfsTransportError } from "../protocol/process.js"
...
import { NativeFailure, type NativeFailureOrTransport } from "./failure.js"
import { loadLinuxLzc } from "./linux.js"
...
export interface NativeBindings {
  ...
  ) => Effect.Effect<ReadonlyArray<DatasetListItem>, NativeFailureOrTransport>
  ...
  ) => Effect.Effect<ReadonlyArray<SnapshotListItem>, NativeFailureOrTransport>
  ...
  ) => Effect.Effect<ReadonlyArray<BookmarkListItem>, NativeFailureOrTransport>
  ...
  /** `ZFS_IOC_REWRITE` on file descriptors. OpenZFS 2.3+. */
  readonly rewrite: (input: Rewrite) => Effect.Effect<void, NativeFailureOrTransport>
  /** `ZFS_IOC_POOL_FREEZE`. Unfreeze with export/import. */
  readonly freezePool: (input: FreezePool) => Effect.Effect<void, NativeFailureOrTransport>
  /** `ZFS_IOC_REMAP` (kernel no-op on current OpenZFS). */
  readonly remap: (input: Remap) => Effect.Effect<void, NativeFailureOrTransport>
  /** `ZFS_IOC_VDEV_SETPATH`. */
  readonly setVdevPath: (input: SetVdevPath) => Effect.Effect<void, NativeFailureOrTransport>
  /** `ZFS_IOC_VDEV_SETFRU`. */
  readonly setVdevFru: (input: SetVdevFru) => Effect.Effect<void, NativeFailureOrTransport>
  /** `ZFS_IOC_INJECT_FAULT` / `zinject`. */
  readonly injectFault: (input: InjectFault) => Effect.Effect<void, NativeFailureOrTransport>
  /** `ZFS_IOC_CLEAR_FAULT`. */
  readonly clearFault: (input: ClearFault) => Effect.Effect<void, NativeFailureOrTransport>
  /** `ZFS_IOC_INJECT_LIST_NEXT`. */
  readonly listFaults: () => Effect.Effect<ReadonlyArray<InjectRecord>, NativeFailureOrTransport>
  /** libzfs `zpool_get_errlog`. */
  readonly errorLog: (input: ErrorLog) => Effect.Effect<ReadonlyArray<ErrorLogRow>, NativeFailureOrTransport>
  /** libzfs `zpool_obj_to_path`. */
  readonly objToPath: (input: ObjToPath) => Effect.Effect<ObjPath, NativeFailureOrTransport>
  /** libzfs `zpool_obj_to_path_ds`. */
  readonly dsobjToName: (input: ObjToPath) => Effect.Effect<ObjPath, NativeFailureOrTransport>
  /** `ZFS_IOC_NEXT_OBJ`. */
  readonly nextObj: (input: NextObj) => Effect.Effect<NextObjResult, NativeFailureOrTransport>
  /** `ZFS_IOC_OBJ_TO_STATS`. */
  readonly objToStats: (input: ObjToStats) => Effect.Effect<ObjStats, NativeFailureOrTransport>
  /** `ZFS_IOC_SMB_ACL`. */
  readonly smbAcl: (input: SmbAcl) => Effect.Effect<void, NativeFailureOrTransport>
}
...
export const unboundBindings = (): NativeBindings => ({
  ...
  rewrite: () => unboundNative("Dataset.Rewrite"),
  freezePool: () => unboundNative("Pool.Freeze"),
  remap: () => unboundNative("Pool.Remap"),
  setVdevPath: () => unboundNative("Pool.SetVdevPath"),
  setVdevFru: () => unboundNative("Pool.SetVdevFru"),
  injectFault: () => unboundNative("Pool.InjectFault"),
  clearFault: () => unboundNative("Pool.ClearFault"),
  listFaults: () => unboundNative("Pool.ListFaults"),
  errorLog: () => unboundNative("Pool.ErrorLog"),
  objToPath: () => unboundNative("Dataset.ObjToPath"),
  dsobjToName: () => unboundNative("Dataset.DsobjToName"),
  nextObj: () => unboundNative("Dataset.NextObj"),
  objToStats: () => unboundNative("Dataset.ObjToStats"),
  smbAcl: () => unboundNative("Mount.SmbAcl")
})
...
export const classifyNativeError = (failure: NativeFailure): ZfsError => {
  ...
  if (candidate) {
    const tags: { readonly [operation: string]: ReadonlyArray<string> } = OperationErrorTags
...
const mapNative = (error: NativeFailureOrTransport): Failure =>
  error instanceof ZfsTransportError ? error : classifyNativeError(error)

const fromNative = <A>(effect: Effect.Effect<A, NativeFailureOrTransport>) => effect.pipe(Effect.mapError(mapNative))
...
export const layerFrom = (bindings: NativeBindings): Layer.Layer<ZfsProtocol> =>
  Layer.succeed(
    ...
    ZfsProtocol.of({
      ...
      rewrite: (input) => fromNative(bindings.rewrite(input)),
      freezePool: (input) => fromNative(bindings.freezePool(input)),
      remap: (input) => fromNative(bindings.remap(input)),
      setVdevPath: (input) => fromNative(bindings.setVdevPath(input)),
      setVdevFru: (input) => fromNative(bindings.setVdevFru(input)),
      injectFault: (input) => fromNative(bindings.injectFault(input)),
      clearFault: (input) => fromNative(bindings.clearFault(input)),
      listFaults: () => fromNative(bindings.listFaults()),
      errorLog: (input) => fromNative(bindings.errorLog(input)),
      objToPath: (input) => fromNative(bindings.objToPath(input)),
      dsobjToName: (input) => fromNative(bindings.dsobjToName(input)),
      nextObj: (input) => fromNative(bindings.nextObj(input)),
      objToStats: (input) => fromNative(bindings.objToStats(input)),
      smbAcl: (input) => fromNative(bindings.smbAcl(input))
    })
  )
...
export const linuxBindings = (): NativeBindings => {
  ...
  if (lzc === undefined) return unbound
  const merged = { ...unbound, ...lzc }
  if (lzc.receive !== undefined) {
    merged.receiveResumable = lzc.receiveResumable ?? lzc.receive
    merged.receiveWithCmdprops = lzc.receiveWithCmdprops ?? lzc.receive
    merged.receiveWithHeal = lzc.receiveWithHeal ?? lzc.receive
  }
  return merged
}
