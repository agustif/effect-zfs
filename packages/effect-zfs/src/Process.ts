...
export const ZfsBinary = Schema.Literals(["zfs", "zpool", "zfsbootenv"])
...
export const command = (binary: ZfsBinary, ...args: string[]): ZfsCommand =>
  new ZfsCommand({ binary, args })
...
export class ZfsProcess extends Context.Service<ZfsProcess, {
  ...
  readonly runWithInput: <E>(command: ZfsCommand, input: Stream.Stream<Uint8Array, E>) => Effect.Effect<CommandResult, ZfsTransportError | E>
