...
import { datasetPropertyNames, poolPropertyNames, vdevPropertyNames } from "../generated/properties.generated.js"
import {
  ByteCount,
  DatasetVersion,
  maxPathBytes,
...
import { BytesOrNone, DatasetKind, PoolHealth } from "../schema/models.js"
import {
  BookmarkComponent,
  BookmarkName,
  DatasetName,
...
export const encodePropertyAssignment = (row: EncodedProperty): string => `${row.name}=${row.value}`
...
export const wrappingKey = (value: string | Uint8Array): WrappingKey => Redacted.make(value)
...
export const wrappingKeyToNativeBytes = (
  ...
  const bytes = typeof material !== "string"
    ? material
    : format === "hex"
    ? decodeHexKey(material)
    : utf8.encode(material)
  if (format !== "passphrase" && bytes.byteLength !== wrappingKeyLen) {
    throw new Error(`wrapping key for ${format} must be ${wrappingKeyLen} bytes`)
  }
  return bytes
}
...
export class CreateFilesystem extends Schema.Class<CreateFilesystem>("effect-zfs/args/CreateFilesystem")({
  ...
  /** CLI `-n`. Native validates and returns without `lzc_create`. */
...
export class Destroy extends Schema.Class<Destroy>("effect-zfs/args/Destroy")({
  ...
  parsable: Schema.optionalKey(Flag),
  verbose: Schema.optionalKey(Flag),
  /** Extra `zfs destroy a@s,b@s` names. */
  names: Schema.optionalKey(Schema.NonEmptyArray(DestroyTarget))
}) {}
...
const encodeLeaves = (leaves: ReadonlyArray<VdevLeaf>): ReadonlyArray<string> => leaves.map((leaf) => leaf.path)
...
export class TrimPool extends Schema.Class<TrimPool>("effect-zfs/args/TrimPool")({
  ...
  /** OpenZFS 2.4 `zpool trim -a`. CLI omits the pool name when set. */
  all: Schema.optionalKey(Flag)
}) {}
...
export class InitializePool extends Schema.Class<InitializePool>("effect-zfs/args/InitializePool")({
  ...
  devices: Schema.optionalKey(Schema.Array(VdevId)),
  wait: Schema.optionalKey(Flag),
  /** OpenZFS 2.4 `zpool initialize -a`. CLI omits the pool name when set. */
  all: Schema.optionalKey(Flag)
}) {}
...
export class Scrub extends Schema.Class<Scrub>("effect-zfs/args/Scrub")({
  ...
  /** OpenZFS 2.4 `zpool scrub -a`. CLI omits the pool name when set. */
  all: Schema.optionalKey(Flag),
  /** OpenZFS 2.4 `zpool scrub -S`. */
  startAfter: Schema.optionalKey(Schema.NonEmptyString),
  /** OpenZFS 2.4 `zpool scrub -E`. */
  endBefore: Schema.optionalKey(Schema.NonEmptyString)
}) {}
...
export { BookmarkComponent, BookmarkName, HoldTag, SnapshotComponent }
...
/** Absolute filesystem path accepted by `zfs unmount` / `zfs unshare` / `zfs rewrite`. */
...
/**
 * `zfs rewrite [-CPSrx] [-l length] [-o offset] file|directory…` (OpenZFS 2.3+,
 * `-P`/`-C` from 2.4). Native: `ZFS_IOC_REWRITE` on each file descriptor.
 * This is a file-path operation, not a dataset name.
 */
export class Rewrite extends Schema.Class<Rewrite>("effect-zfs/args/Rewrite")({
  files: Schema.NonEmptyArray(AbsolutePath),
  recursive: Schema.optionalKey(Flag),
  verbose: Schema.optionalKey(Flag),
  xdev: Schema.optionalKey(Flag),
  /** OpenZFS 2.4 `-P`. Preserve logical birth time. */
  physical: Schema.optionalKey(Flag),
  /** Skip snapshot-shared blocks (`ZFS_REWRITE_SKIP_SNAPSHOT`). */
  skipSnapshot: Schema.optionalKey(Flag),
  /** OpenZFS 2.4 `-C`. Skip BRT-cloned blocks. */
  skipBrt: Schema.optionalKey(Flag),
  offset: Schema.optionalKey(ByteCount),
  length: Schema.optionalKey(ByteCount)
}) {}
...
/** `zpool freeze`. Native: `ZFS_IOC_POOL_FREEZE`. Unfreeze with export/import. */
export class FreezePool extends Schema.Class<FreezePool>("effect-zfs/args/FreezePool")({
  name: PoolName
}) {}

/** `ZFS_IOC_REMAP` (kernel no-op on current OpenZFS). */
export class Remap extends Schema.Class<Remap>("effect-zfs/args/Remap")({
  name: DatasetName
}) {}

/** `ZFS_IOC_VDEV_SETPATH`. */
export class SetVdevPath extends Schema.Class<SetVdevPath>("effect-zfs/args/SetVdevPath")({
  pool: PoolName,
  vdev: Schema.NonEmptyString,
  path: Schema.NonEmptyString
}) {}

/** `ZFS_IOC_VDEV_SETFRU`. */
export class SetVdevFru extends Schema.Class<SetVdevFru>("effect-zfs/args/SetVdevFru")({
  pool: PoolName,
  vdev: Schema.NonEmptyString,
  fru: Schema.NonEmptyString
}) {}

export const InjectKind = Schema.Literals(["io", "checksum", "delay", "panic", "flush", "unload"])
export type InjectKind = typeof InjectKind.Type

/** `zinject` / `ZFS_IOC_INJECT_FAULT`. */
export class InjectFault extends Schema.Class<InjectFault>("effect-zfs/args/InjectFault")({
  pool: PoolName,
  kind: InjectKind,
  device: Schema.optionalKey(Schema.NonEmptyString),
  object: Schema.optionalKey(UInt64),
  duration: Schema.optionalKey(Schema.Number)
}) {}

/** `ZFS_IOC_CLEAR_FAULT`. */
export class ClearFault extends Schema.Class<ClearFault>("effect-zfs/args/ClearFault")({
  id: UInt64
}) {}

export class InjectRecord extends Schema.Class<InjectRecord>("effect-zfs/InjectRecord")({
  id: UInt64,
  pool: Schema.optionalKey(PoolName),
  kind: Schema.optionalKey(InjectKind)
}) {}

const injectKindOf = (raw: string): InjectKind | undefined => {
  const token = raw.toLowerCase()
  if (token === "io" || token.includes("zio")) return "io"
  if (token.includes("checksum")) return "checksum"
  if (token.includes("delay")) return "delay"
  if (token.includes("panic")) return "panic"
  if (token.includes("flush")) return "flush"
  if (token.includes("unload")) return "unload"
  return undefined
}

type ZinjectSection = "device" | "delay" | "data" | "panic" | "pool-delay"

const zinjectSectionOf = (header: string): ZinjectSection => {
  const token = header.toLowerCase()
  if (token.includes("function")) return "panic"
  if (token.includes("delay") && token.includes("command")) return "pool-delay"
  if (token.includes("delay")) return "delay"
  if (token.includes("objset")) return "data"
  return "device"
}

const zinjectKindOfSection = (
  section: ZinjectSection | undefined,
  columns: ReadonlyArray<string>
): InjectKind | undefined => {
  if (section === "delay" || section === "pool-delay") return "delay"
  if (section === "panic") return "panic"
  let kind: InjectKind | undefined
  for (const column of columns) {
    const found = injectKindOf(column)
    if (found !== undefined) kind = found
  }
  return kind
}

/** Parse `zinject` handler listing into typed rows. */
export const parseZinjectList = (stdout: string): ReadonlyArray<InjectRecord> => {
  const rows: Array<InjectRecord> = []
  let section: ZinjectSection | undefined
  for (const line of stdout.split("\n")) {
    const trimmed = line.trim()
    if (
      trimmed === ""
      || trimmed.startsWith("---")
      || /^no handlers/i.test(trimmed)
      || /^run 'zinject/i.test(trimmed)
    ) {
      continue
    }
    if (/^id\b/i.test(trimmed)) {
      section = zinjectSectionOf(trimmed)
      continue
    }
    const columns = trimmed.split(/\s+/)
    const idRaw = columns[0]
    if (idRaw === undefined || !/^\d+$/.test(idRaw)) continue
    const poolRaw = columns[1]
    const named = poolRaw !== undefined && poolWhy(poolRaw) === undefined ? poolName(poolRaw) : undefined
    const kind = zinjectKindOfSection(section, columns)
    rows.push(
      new InjectRecord({
        id: uint64(BigInt(idRaw)),
...
export class ObjToPath extends Schema.Class<ObjToPath>("effect-zfs/args/ObjToPath")({
  pool: PoolName,
  datasetObject: UInt64,
  object: UInt64
}) {}

export class NextObj extends Schema.Class<NextObj>("effect-zfs/args/NextObj")({
  dataset: DatasetName,
  object: Schema.optionalKey(UInt64)
}) {}

export class ObjToStats extends Schema.Class<ObjToStats>("effect-zfs/args/ObjToStats")({
  dataset: DatasetName,
  object: UInt64
}) {}

export class ObjStats extends Schema.Class<ObjStats>("effect-zfs/ObjStats")({
  path: Schema.NonEmptyString,
  gen: Schema.optionalKey(UInt64),
  size: Schema.optionalKey(ByteCount)
}) {}

export const SmbAclAction = Schema.Literals(["add", "remove", "rename", "xattr"])
export type SmbAclAction = typeof SmbAclAction.Type

export class SmbAcl extends Schema.Class<SmbAcl>("effect-zfs/args/SmbAcl")({
  dataset: DatasetName,
  action: SmbAclAction,
  path: Schema.optionalKey(Schema.NonEmptyString)
}) {}

export class ErrorLogRow extends Schema.Class<ErrorLogRow>("effect-zfs/ErrorLogRow")({
  name: Schema.NonEmptyString,
  object: Schema.optionalKey(UInt64)
}) {}

/** Parse the permanent-errors list from `zpool status -v`. */
export const parseStatusErrorLog = (stdout: string): ReadonlyArray<ErrorLogRow> => {
  const match = /^errors:\s*(.*)$/im.exec(stdout)
  if (match === null) return []
  const summary = (match[1] ?? "").trim().toLowerCase()
  if (summary.startsWith("no known")) return []
  const start = (match.index ?? 0) + match[0].length
  const rest = stdout.slice(start)
  const rows: Array<ErrorLogRow> = []
  for (const line of rest.split("\n")) {
    ...
    if (name === "") continue
    const objectMatch = /:<0x([0-9a-f]+)>$/i.exec(name)
    const object = objectMatch?.[1] === undefined ? undefined : uint64(BigInt(`0x${objectMatch[1]}`))
    rows.push(
      new ErrorLogRow({
        name,
...
/** `zpool_get_errlog` / `ZFS_IOC_ERROR_LOG`. */
export class ErrorLog extends Schema.Class<ErrorLog>("effect-zfs/args/ErrorLog")({
  name: PoolName
}) {}

export class ObjPath extends Schema.Class<ObjPath>("effect-zfs/ObjPath")({
  path: Schema.NonEmptyString
}) {}

export class NextObjResult extends Schema.Class<NextObjResult>("effect-zfs/NextObjResult")({
  object: UInt64
}) {}
