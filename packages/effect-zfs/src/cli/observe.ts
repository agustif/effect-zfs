...
import { EventsCleared, HistoryRecord, IostatRow, IostatSample, PoolEvent } from "../args/index.js"
import { byteCount } from "../schema/limits.js"
import { poolName, poolWhy } from "../schema/name.js"
...
export const eventsFromLines = <E, R>(lines: Stream.Stream<string, E, R>): Stream.Stream<PoolEvent, E, R> =>
  lines.pipe(
    Stream.mapAccum(
      ...
      {
        onHalt: (current) => current === undefined ? [] : [toEvent(current.time, current.eventClass, current.payload)]
      }
    )
  )
