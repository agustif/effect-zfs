...
import type { DeviceOpenFailed, PoolInvalidArgument, PoolNotSupported } from "../generated/errors.generated.js"
import {
  BadAttachTarget,
  BadPath,
  BadRestore,
...
import { type CommandResult, ZfsCommand } from "../protocol/process.js"
...
const base = (operation: string, stderr: string) => ({
  operation,
  message: stderr.trim() || "ZFS command failed",
  stderr
})
const has = (s: string, pattern: RegExp) => pattern.test(s.toLowerCase())
const unknown = (operation: string, result: CommandResult) =>
  new UnknownZfsError({
    operation,
...
const classifyKnown = (operation: string, result: CommandResult): ClassifiedZfsError | undefined => {
  ...
  if (has(s, /invalid property|bad property|unknown property/)) {
    return new InvalidProperty({ code: "EZFS_BADPROP", ...b })
  }
  if (has(s, /read.?only property|is readonly|property is readonly/)) {
    return new PropertyReadOnly({ code: "EZFS_PROPREADONLY", ...b })
  }
  if (has(s, /property.*does not apply|property.*not.*applicable|does not apply to datasets/)) {
    return new PropertyNotApplicable({ code: "EZFS_PROPTYPE", ...b })
  }
  if (has(s, /property.*cannot be inherited|not inheritable/)) {
    return new PropertyNotInheritable({ code: "EZFS_PROPNONINHERIT", ...b })
  }
  if (has(s, /currently resilvering|cannot restart resilver/)) {
    return new Resilvering({ code: "EZFS_RESILVERING", ...b })
  }
  if (has(s, /dataset.*busy|pool.*busy|currently being scrubbed|already in progress|being held/)) {
  ...
  if (has(s, /does not exist|no such pool|no such dataset|cannot open.*no such/)) {
    return new DatasetNotFound({ code: "EZFS_NOENT", ...b })
  }
  ...
  if (
    has(
      s,
      /incorrect key|encryption failure|failed to setup encryption|key load error|could not load key|wrapping key|encryption currently not supported|crypto failed/
    )
  ) {
 ...
 * union. Native `layerFrom` bypasses this heuristic and maps
...
export const classifyCliError = (operation: string, result: CommandResult): ZfsError => {
  ...
  const declared = OperationErrorTags[operation as keyof typeof OperationErrorTags] as ReadonlyArray<string> | undefined
